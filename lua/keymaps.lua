...
vim.keymap.set("n", "<leader>ss", function() builtin.current_buffer_fuzzy_find() end,
  { desc = "String in current buffer (Fuzzy)" })
...
vim.keymap.set("n", "<leader>sF", function() builtin.find_files({ no_ignore = true }) end,
  { desc = "Search for files (include ignored files)" })
...
vim.keymap.set("n", "<leader>sA", function() builtin.live_grep({ additional_args = { "--no-ignore" } }) end,
  { desc = "Search for string in all files (include ignored files)" })
vim.keymap.set("n", "<leader>sk", ":lua require'telescope.builtin'.keymaps(require('telescope.themes').get_ivy({}))<cr>",
...
vim.api.nvim_create_autocmd("LspAttach", {
  ...
  callback = function(ev)
    ...
    vim.keymap.set("n", "<leader>li", vim.lsp.buf.implementation, { buffer = ev.buf, desc = "Go to implementation" })
    ...
    vim.keymap.set("n", "<leader>la", vim.lsp.buf.code_action, { buffer = ev.buf, desc = "See available code actions" })
    vim.keymap.set("n", "<leader>lR", "<cmd>telescope lsp_references<cr>",
      { buffer = ev.buf, desc = "Show LSP references" })
    vim.keymap.set("n", "<leader>dl", "<cmd>TroubleToggle document_diagnostics<cr>",
      { buffer = ev.buf, desc = "Document diagnostics list (Trouble)" })
    vim.keymap.set("n", "<leader>dL", "<cmd>TroubleToggle workspace_diagnostics<cr>",
